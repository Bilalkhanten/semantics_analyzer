 ...
 * ----------------
 *
 * Stores new value for given identifier. If the key already has an
 * entry and flag is to overwrite, will remove previous entry first,
 ...
 * changes are made.  Does not affect any other entries under that
 * key.
 */
 ...
 * -----------------
 *
 * Returns the value earlier stored under key or NULL if there is no
 * matching entry
 */
template <class Value> 
 ...
 * ---------------------
 *
 * Returns iterator which can be used to walk through all values in
 * table.
 */
template <class Value> 
 ...
 * ----------------------
 *
 * Iterator function used to return current value and advance iterator
