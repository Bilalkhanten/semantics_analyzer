 ...
 * Function: yyerror()
 * -------------------
 * Standard error-reporting function expected by yacc. Our version
 * merely just calls into the error reporter above, passing the
 * location of the last token read. If you want to suppress the
 * ordinary "parse error" message from yacc, you can implement yyerror
 * to do nothing and then call ReportError::Formatted yourself with a
