 ...
 * version if you so prefer.  You should not need to make any
 * significant changes in the parser itself. After parsing completes,
 * if no syntax errors were found, the parser calls program->Check()
 * to kick off the semantic analyzer pass. The interesting work
 * happens during the tree traversal.
 */
%{
...
Expr      :    LValue '=' Expr      { Operator *op = new Operator(@2, "=");
          ...
          |    Expr T_NotEqual Expr { Operator *op = new Operator(@2, "!=");
                                      $$ = new EqualityExpr($1, op, $3);
                                    }
          |    Expr T_And Expr      { Operator *op = new Operator(@2, "&&");
                                      $$ = new LogicalExpr($1, op, $3);
                                    }
          |    Expr T_Or Expr       { Operator *op = new Operator(@2, "||");
                                      $$ = new LogicalExpr($1, op, $3);
                                    }
          |    '!' Expr             { Operator *op = new Operator(@1, "!");
